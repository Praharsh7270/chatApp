    ...
    <div className={`bg-[#818582]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
        <div className='pb-5 '>
            ...
            <div className='bg-[#2f3136] rounded-full flex items-center gap-3 px-3 py-3 mt-5'>
                ...
                <input type="text" className='bg-transparent border-none outline-none text-white text-xs placeholder-[#c8c8c8] flex-1' placeholder='Seach'/>
            </div>
        </div>
            ...
            {userDummyData.map((user, index) =>(
                <div onClick={() =>{
                    ...
                    <div className='flex flex-col leading-5'>
                        <p>{user.fullName}</p>
                        {
                            index < 3 ? <span className='text-green-500'>online</span>  : <span className='text-red-500'>offline</span>
                        }
                    </div>
